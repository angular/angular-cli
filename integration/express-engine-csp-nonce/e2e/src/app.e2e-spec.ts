...
describe('Hello world E2E Tests', () => {
  ...
  it('stylesheets should be configured to load asynchronously', async () => {
    ...
    expect(linkTag.getAttribute('ngCspMedia')).toMatch(`all`);
    expect(linkTag.getAttribute('onload')).toBeNull();
  ...
  it('style tags all have a nonce attribute', async () => {
    ...
    // Test the contents from the server.
    const styleTags = await browser.driver.findElements(by.css('span'));
    expect(styleTags.every((e) => e.getAttribute('nonce'))).toBe(true);

    // Make sure there were no client side errors.
    verifyNoBrowserErrors();
  });

  it('script tag to add onload handler all have a nonce attribute', async () => {
    // Load the page without waiting for Angular since it is not bootstrapped automatically.
    await browser.driver.get(browser.baseUrl);
