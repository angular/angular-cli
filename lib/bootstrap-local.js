      ...
      if (match) {
        ...
        if (maybeTsPath && !request.endsWith('.json')) {
          ...
          if (fs.existsSync(maybeTsPath)) {
            ...
            // the method directly.
            const tmpJsonSchemaPath = path.join(
              tmpRoot,
              maybeTsPath.replace(/[^0-9a-zA-Z.]/g, '_'),
            );
            try {
              if (!fs.existsSync(tmpJsonSchemaPath)) {
                const quicktypeRunnerPath = path.join(__dirname, '../tools/quicktype_runner.ts');
                child_process.spawnSync('tsc', [quicktypeRunnerPath, resolved, tmpJsonSchemaPath]);
              }
