...
class Angular2App extends BroccoliPlugin {
  constructor(project, inputNode, options) {
    ...
    this._options.prodPolyfills = this._options.prodPolyfills || [
  ...
  _getIndexTree() {
    ...
    return new HandlebarReplace(indexTree, {
      ...
      scripts: {
        ...
        prodPolyfills: this._options.prodPolyfills
      },
      mobile: mobile
    }, {
      helpers: {
  ...
  _getBundleTree(preBundleTree){
    ...
    var prodScriptTree = new BroccoliFunnel(preBundleTree, {
      include: this._options.prodPolyfills
    });
    ...
    if (this.ngConfig.apps[0].mobile) {
      bundleTree = concat(BroccoliMergeTrees([vendorTree, jsTree, scriptTree, prodScriptTree, bundleTree], {
        overwrite: true
      }), {
    ...
    return BroccoliMergeTrees([nonJsTree, scriptTree, prodScriptTree, bundleTree], { overwrite: true });
  }
}
