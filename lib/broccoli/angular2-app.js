...
class Angular2App extends BroccoliPlugin {
  ...
  /**
   * Build the input tree based on the project default configuration.
   * This includes; all TS files, all assets and the public folder.
  ...
  _buildTree() {
    ...
    var publicTree = this._getPublicTree();

    var buildTrees = [assetTree, tsTree, indexTree, vendorNpmTree];

    if (fs.existsSync('public')) {
      buildTrees.push(publicTree);
    }

    for (var name of ['sass', 'less', 'stylus', 'compass']) {
      var plugin = require(`./angular-broccoli-${name}`);
      var tree = plugin.makeBroccoliTree(this._inputNode, this._options[`${name}Compiler`]);
  ...
  _getTypingsTree() {
    return new BroccoliFunnel(this._inputNode, {
      include: ['typings/browser.d.ts', 'typings/browser/**'],
      name: 'typings-tree'
    });
  }
   ...
   * @return {Funnel} The dotfiles exclusion tree.
   */
  _getPublicTree() {
    return new BroccoliFunnel(this._inputNode, {
      srcDir: 'public',
