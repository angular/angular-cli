...
class Angular2SharePointApp extends BroccoliPlugin {
   ...
   * - Get the TS tree
   * - Get the TS src tree
   * - Get the CustomMaster.html tree 
  ...
  _buildTree() {
    ...
    if (this.ngConfig.apps[0].mobile) {
      ...
      merged = new BroccoliMergeTrees([merged, new AppShellPlugin(merged, 'CustomMaster.html', 'main-app-shell')], {
        overwrite: true
      });
    }
  ...
  /**
   * Returns the content for a specific type (section) for CustomMaster.html.
   *
  ...
  /**
   * Returns the tree for app/CustomMasterhtml.
   *
   * @private
   * @method _getIndexTree
   * @return {Tree} Tree for app/CustomMaster.html.
   */
  _getIndexTree() {
    var files = [
      'CustomMaster.html'
