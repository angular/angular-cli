...
class DiffingTSCompiler extends Plugin {
  constructor(inputPath, options) {
    ...
    if (options.files) {
      this.rootFilePaths = options.files.splice(0);
    } else {
      this.rootFilePaths = [];
    }
  ...
  createServiceHost() {
    ...
    this.tsOpts = ts.parseJsonConfigFileContent(this.options, null, null).options;
    ...
    this.tsServiceHost = new CustomLanguageServiceHost(
      this.tsOpts, this.rootFilePaths, this.fileRegistry, this.inputPaths[0]);
    this.tsService = ts.createLanguageService(this.tsServiceHost, ts.createDocumentRegistry());
  }

  build() {
    if (!this.tsServiceHost) {
    ...
    entries.forEach(function (entry) {
      const tsFilePath = path.join(_this.inputPaths[0], entry.relativePath);
      // const entry = entries.filter(e => e.relativePath == tsFilePath)[0];
      if (!tsFilePath.match(/\.ts$/)) {
        return;
      }
      if (!_this.fileRegistry[tsFilePath]) {
        _this.fileRegistry[tsFilePath] = { version: entry.mtime };
      } else if (_this.fileRegistry[tsFilePath].version >= entry.mtime) {
        // Nothing to do for this file. Just reoutput.
      } else {
        _this.fileRegistry[tsFilePath].version = entry.mtime;
      }
      pathsEmitted.push(tsFilePath);
      ...
      if (output.emitSkipped) {
        ...
        output.outputFiles.forEach(function (o) {
          ...
          if (path.isAbsolute(filePath)) {
            filePath = filePath.replace(_this.inputPaths[0], _this.outputPath);
          } else {
            filePath = path.join(_this.outputPath, filePath);
          }
          var destDirPath = path.dirname(filePath);
          fse.mkdirsSync(destDirPath);
          fs.writeFileSync(filePath, _this.fixSourceMapSources(o.text), FS_OPTS);
        });
      }
    });
    ...
    if (pathsWithErrors.length) {
      ...
      tsEmitInternal = true;
      pathsEmitted.forEach(function (tsFilePath) {
        var output = _this.tsService.getEmitOutput(tsFilePath);
        if (!output.emitSkipped) {
          output.outputFiles.forEach(function (o) {
            var filePath = o.name;
  ...
  doFullBuild() {
    ...
    tsEmitInternal = false;
    var emitResult = program.emit(undefined, (absoluteFilePath, fileContent) => {
      absoluteFilePath = path.resolve(this.outputPath, absoluteFilePath);
      // Replace the input path by the output.
      absoluteFilePath = absoluteFilePath.replace(this.inputPaths[0], this.outputPath);
  ...
  removeOutputFor(tsFilePath) {
    var absoluteJsFilePath = path.join(this.outputPath, tsFilePath.replace(/\.ts$/, '.js'));
    var absoluteMapFilePath = path.join(this.outputPath, tsFilePath.replace(/.ts$/, '.js.map'));
    var absoluteDtsFilePath = path.join(this.outputPath, tsFilePath.replace(/\.ts$/, '.d.ts'));
...
var CustomLanguageServiceHost = (function () {
  ...
  CustomLanguageServiceHost.prototype.getCurrentDirectory = function () {
    return this.currentDirectory;
  };
  CustomLanguageServiceHost.prototype.getCompilationSettings = function () {
    return this.compilerOptions;
  };
  CustomLanguageServiceHost.prototype.getDefaultLibFileName = function () {
...
exports['default'] = DiffingTSCompiler;
