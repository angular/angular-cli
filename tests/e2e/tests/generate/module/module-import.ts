...
export default function () {
  ...
  const modulePath = join(root, 'projects', 'test-project', 'src', 'app', 'app.module.ts');
  const subModulePath = join('projects', 'test-project', 'src', 'app', 'sub', 'sub.module.ts');
  const deepSubModulePath = join('projects', 'test-project', 'src', 'app', 'sub', 'deep', 'deep.module.ts');
  ...
  return Promise.resolve()
    ...
    .then(() => expectFileToMatch(join('projects', 'test-project', 'src', 'app', 'test4', 'test4.module.ts'),
    ...
    .then(() => process.chdir(join(root, 'projects', 'test-project', 'src', 'app')))
