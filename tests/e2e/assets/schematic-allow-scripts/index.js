...
exports.default = ({ allowScripts, ignoreScripts = false }) => {
  return (tree, context) => {
    ...
    tree.create(
      '/install-test/run-post.js',
      'require("fs").writeFileSync(__dirname + "/post-script-ran", "12345");',
    );
    context.addTask(
      new tasks.NodePackageInstallTask({ workingDirectory: 'install-test', allowScripts }),
    );
  };
};
