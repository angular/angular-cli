...
async function verifyMedia(fileNameRe: RegExp, content: RegExp) {
  const fileName = await expectFileMatchToExist('dist/test-project/browser/', fileNameRe);
  await expectFileToMatch(`dist/test-project/browser/${fileName}`, content);
}
...
export default async function () {
  ...
  await expectFileToMatch('dist/test-project/browser/index.html', /runtime\.[0-9a-f]{16}\.js/);
  await expectFileToMatch('dist/test-project/browser/index.html', /main\.[0-9a-f]{16}\.js/);
  await expectFileToMatch('dist/test-project/browser/index.html', /styles\.[0-9a-f]{16}\.(css|js)/);
  ...
  await expectFileToMatch('dist/test-project/browser/index.html', /runtime\.js/);
  await expectFileToMatch('dist/test-project/browser/index.html', /main\.js/);
  await expectFileToMatch('dist/test-project/browser/index.html', /styles\.(css|js)/);
