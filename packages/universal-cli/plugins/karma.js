...
const init = (config) => {
  ...
  // replace the universal-cli preprocessor with webpack+sourcemap
  Object.keys(config.preprocessors)
    .filter((file) => config.preprocessors[file].indexOf('universal-cli') !== -1)
    .map((file) => config.preprocessors[file])
    .map((arr) => arr.splice(arr.indexOf('universal-cli'), 1, 'webpack', 'sourcemap'));
}
...
module.exports = Object.assign({
  'framework:universal-cli': ['factory', init],
  'preprocessor:universal-cli': ['factory', preprocessor]
