...
export default Blueprint.extend({
  ...
  availableOptions: [
    ...
    },
    {
      ...
      description: 'Specifies where the resolver should be provided.'
    },
    {
      name: 'app',
      type: String,
  ...
  locals: function (options: any) {
    options.flat = options.flat !== undefined ?
      options.flat : CliConfig.getValue('defaults.resolver.flat');

    options.spec = options.spec !== undefined ?
      options.spec : CliConfig.getValue('defaults.resolver.spec');
  ...
  files: function () {
    ...
    if (this.options && !this.options.spec) {
      fileList = fileList.filter(p => p.indexOf('__name__.resolver.spec.ts') < 0);
    }
  ...
  afterInstall(options: any) {
    ...
    if (!this.pathToModule) {
      const warningMessage = oneLine`
        Resolver is generated but not provided,
      ...
      const className = stringUtils.classify(`${options.entity.name}Resolver`);
      const fileName = stringUtils.dasherize(`${options.entity.name}.resolver`);
