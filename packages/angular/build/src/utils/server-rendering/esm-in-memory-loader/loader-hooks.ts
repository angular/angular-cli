...
import { transformWithBabel } from '../../../tools/babel/transform';
...
export async function load(url: string, context: { format?: string | null }, nextLoad: Function) {
  ...
  if (format === 'module' && isFileProtocol(url)) {
    ...
    if (filePath.includes('@angular/')) {
      ...
      content = Buffer.concat([NG_SERVER_MODE_INIT_BYTES, content]);
    }

    const source = await transformWithBabel(
      filePath,
      content,
      // Always enable JIT linking to support applications built with and without AOT.
      // In a development environment the additional scope information does not
      // have a negative effect unlike production where final output size is relevant.
      { sourcemap: true, jit: true, advancedOptimizations: false, thirdPartySourcemaps: false },
    );
