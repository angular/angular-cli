...
import { maxInlinerWorkers, maxTransformWorkers } from './environment-options';
...
/**
 * Returns the singleton shared build worker pool instance bounded by `maxInlinerWorkers`.
...
export function getSharedBuildWorkerPool(): WorkerPool {
  if (!sharedBuildWorkerPool) {
    ...
    sharedBuildWorkerPool = new WorkerPool({
      ...
      maxThreads: maxInlinerWorkers,
      minThreads: maxTransformWorkers,
