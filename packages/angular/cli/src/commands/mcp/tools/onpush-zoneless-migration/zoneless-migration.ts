...
export async function registerZonelessMigrationTool(
  ...
  const componentTestFiles = new Set<ts.SourceFile>();
  const filesWithComponents = new Set<ts.SourceFile>();
  ...
  if (fs.statSync(fileOrDirPath).isDirectory()) {
    ...
    if (maybeTestFile) {
      componentTestFiles.add(createSourceFile(maybeTestFile));
    }
  }
  ...
  for (const sourceFile of files) {
    ...
    if (testBedSpecifier) {
      ...
      if (testFilePath) {
        componentTestFiles.add(createSourceFile(testFilePath));
      }
  ...
  if (zoneFiles.size > 0) {
    for (const file of zoneFiles) {
      const result = analyzeForUnsupportedZoneUses(file);
      if (result !== null) {
        return result;
      }
    }
