 ...
 * `@angular/core`. It achieves this by reading a custom `angular-mcp` metadata block in the
 ...
 * version of `@angular/core`. It looks for a custom `angular-mcp` metadata property in the
 * framework's `package.json` to locate the guide.
 *
 * @example A sample `package.json` `angular-mcp` field:
 * ```json
 * {
 *   "angular-mcp": {
 *     "bestPractices": {
 *       "format": "markdown",
 *       "path": "./mcp-resources/best-practices.md"
 *     }
 *   }
 * }
 * ```
...
async function getVersionSpecificBestPractices(
  ...
  try {
    ...
    const bestPracticesInfo = pkgJson['angular-mcp']?.bestPractices;
    ...
    if (
      ...
      logger.warn(
        `Did not find valid 'angular-mcp.bestPractices' metadata in '${pkgJsonPath}'. ` +
          'Falling back to the bundled guide.',
      );
    }
  } catch (e) {
