...
export default async function(options: { testing?: boolean, cliArgs: string[] }) {
  ...
  try {
    ...
    if (options.cliArgs.includes('e2e') 
        && options.cliArgs[options.cliArgs.findIndex('-force')+1] === "true") {
      logger.fatal('An unexpected error occurred while e2e run: ' 
                   + JSON.stringify(err) + ';  Used --force to continue');
      return 0;
