...
describe('ConsoleLoggerStack', () => {
  ...
  it('works as a stack', (done: DoneFn) => {
    ...
    logger
      ...
      .then((observed: LogEntry[]) => {
        expect(observed).toEqual([
          jasmine.objectContaining({ message: 'red', level: 'info', name: 'test' }) as any,
          jasmine.objectContaining({ message: 'blue', level: 'info', name: 'test2' }) as any,
          jasmine.objectContaining({ message: 'yellow', level: 'info', name: 'test3' }) as any,
          jasmine.objectContaining({ message: 'green', level: 'info', name: 'test2' }) as any,
        ]);
      })
      .then(() => done(), (err: any) => done.fail(err));
