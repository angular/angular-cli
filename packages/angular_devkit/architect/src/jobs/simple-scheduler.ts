    ...
    return handler.pipe(
      switchMap((handler) => {
        ...
        const handlerWithExtra = Object.assign(handler.bind(undefined), {
          ...
          argumentV: this._schemaRegistry.compileAsync(description.argument),
          inputV: this._schemaRegistry.compileAsync(description.input),
          outputV: this._schemaRegistry.compileAsync(description.output),
