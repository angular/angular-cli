...
function findLocalizePositions(
  ...
  const file = new File({filename: options.filename}, { code: options.code, ast });
    ...
    traverse(file.ast, {
      ...
      try {
        ...
        if (
          ...
          if (utils.isBabelParseError(e)) {
            // If we get a BabelParseError here then there must be something wrong with the
            // structure of the AST generated by Babel parsing a TaggedTemplateExpression.
            throw utils.buildCodeFrameError(path as any, e);
          } else {
            throw e;
          }
        }
    ...
    traverse(file.ast, {
      TaggedTemplateExpression(path) {
        try {
          ...
          if (utils.isBabelParseError(e)) {
            // If there was an error processing the AST, make sure we show where the error
            // occurred.
            throw utils.buildCodeFrameError(path as any, e);
          } else {
            throw e;
