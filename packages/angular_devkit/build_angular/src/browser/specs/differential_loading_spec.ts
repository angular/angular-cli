...
describe('Browser Builder with differential loading', () => {
  ...
  it('uses the right zone.js variant', async () => {
    const { files } = await browserBuild(architect, host, target, { optimization: false });
    expect(await files['polyfills-es5.js']).toContain('zone.js/bundles/zone-legacy.umd');
    expect(await files['polyfills-es5.js']).toContain('registerElementPatch');
    // expect(await files['polyfills-es5.js']).toContain('zone.js');
    // expect(await files['polyfills-es2015.js']).toContain('zone.js');
    expect(await files['polyfills-es2015.js']).not.toContain('zone.js/bundles/zone-legacy.umd');
