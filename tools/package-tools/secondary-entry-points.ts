...
export function getSecondaryEntryPointsForPackage(pkg: BuildPackage) {
  ...
  const entryPoints = getSubdirectoryNames(packageDir)
    .filter(d => existsSync(join(packageDir, d, 'tsconfig.lib.json')));
...
function buildPackageImportStatementFindCommand(searchDirectory: string, packageName: string) {
  if (platform() === 'win32') {
    return {
      binary: 'findstr',
      args: ['/r', `from.'@${buildConfig.namespace}/${packageName}/.*'`, `${searchDirectory}\\*.ts`]
    };
  } else {
    return {
      binary: 'grep',
      args: ['-Eroh', '--include', '*.ts', `from '@${buildConfig.namespace}/${packageName}/.+';`,
        searchDirectory]
    };
  }
}
