...
export class PackageBundler {
  ...
  private async bundlePrimaryEntryPoint() {
    ...
    const dashedPackageName = dashCaseToCamelCase(packageName);
    ...
    return this.bundleEntryPoint({
      ...
      importName: `@${buildConfig.namespace}/${packageName}`,
      moduleName: `${buildConfig.namespace}.${dashedPackageName}`,
  ...
  private async bundleSecondaryEntryPoint(entryPoint: string) {
    ...
    return this.bundleEntryPoint({
      entryFile,
      esm5EntryFile,
      importName: `@${buildConfig.namespace}/${packageName}/${dashedEntryName}`,
      moduleName: `${buildConfig.namespace}.${dashedPackageName}.${dashedEntryName}`,
  ...
  private async createRollupBundle(config: RollupBundleConfig) {
    ...
    const writeOptions = {
      name: config.moduleName || `${buildConfig.namespace}.common`,
    ...
    if (config.format === 'umd') {
      ...
      if (this.buildPackage.exportsSecondaryEntryPointsAtRoot &&
        config.moduleName === `${buildConfig.namespace}.${this.buildPackage.name}`) {

        const importRegex = new RegExp(`@${buildConfig.namespace}/${this.buildPackage.name}/.+`);
  ...
  private getResolvedSecondaryEntryPointImportPaths(bundleOutputDir: string) {
    return this.buildPackage.secondaryEntryPoints.reduce((map, p) => {
      map[`@${buildConfig.namespace}/${this.buildPackage.name}/${p}`] =
